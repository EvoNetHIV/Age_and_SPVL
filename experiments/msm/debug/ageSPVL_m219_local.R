...
#Run model for each value of iispecified target stats
for(ii in 1:length(variable_values)){
  ...
  param_list=list(
    ...
    nw_form_terms = "~edges+absdiff('sqrt_age') + concurrent(by=~age>25) + 
                        offset(nodematch('role',diff=TRUE, keep=1:2))",
    target_stats = c(initial_pop*meandeg/2, mean_sqrt_age_diff*initial_pop*meandeg/2,
                     0.2*initial_pop*0.18, 0.02*initial_pop*0.82),          
                    # 20% conc for the 18% of agents under age 25, 2% for the 82% over 25
