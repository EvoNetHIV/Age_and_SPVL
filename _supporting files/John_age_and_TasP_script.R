...
hyak_path= '/gscratch/csde/jmittler/exp41'
local_path="~/exp41"

if(hyak)outpath=hyak_path else 
  outpath=local_path

nw_path=file.path(outpath,"evo_nw_start.RData")
...
num_superreps = 1 # If > 1, will do additional replicates with randomly (or systematically) choosen parameters
...
param_vec = list(1,2,3,4,5,6,7,8,10,12,14,16,18,20)
...
{
  abs_diff_age = param_vec[jj][[1]]   # Difference in age
  abs_diff_age_param <- tot_edges * abs_diff_age
  evoparams$target_stats <- c(tot_edges, 2*tot_edges*f1*f2, tot_edges*f2*f2, 2*tot_edges*f1*f3, 2*tot_edges*f2*f3, tot_edges*f3*f3,
                              abs_diff_age_param)#0.06*tot_edges/2)  # last one (blanked out here) was a concurrency term
  # STUFF FROM SARAHs scrpt
  ...
  est <- do.call(EpiModel::netest, netest_arg_list)
  save(est,file=nw_path)
  
  for(ii in 1:4)
  {  
    evoparams$tx_type <- criterion_vec[ii]
    model_name = paste("txt_opto_10_",criterion_names[ii],"_agediff",param_names[jj],sep="")
    
    evoparams$proportion_treated  = 0.6
    ...
    evo_module_list<- list(
      ...
      #  "scale_up.FUN"       = social_increased_testing_selected_groups_module,
      ...
      #"update_cd4_FUN"     = evonet:::viral_update_cd4_daily_with_age, # viral_update_cd4_diff_eqn, viral_update_cd4_simple_diff_eqn, 
    ...
    #--------------------------------------------------------------
    if(isTRUE(hyak_par)){
      evomodel  <- EpiModelHPC::netsim_hpc(x = nw_path, 
                                           param = evoparams, 
                                           init = infected_list, 
                                           save.min = FALSE,
                                           save.max = FALSE,
